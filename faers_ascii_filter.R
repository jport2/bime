...
drug_reac_b13a <- drug_reac_b13 %>% 
  group_by(ISR_id) %>% 
  mutate(drug_names = paste0(DRUGNAME1, collapse = " "),
         pt_names = paste0(PT1, collapse=" ")) 
...
write.table(drug_reac_b13c, file="drug_reac_b13_full.txt", row.names=FALSE, quote=FALSE, sep=" ")
write.csv(drug_reac_b13c, file="drug_reac_b13_full.csv", row.names=FALSE, quote=FALSE)
...
write.table(dfdr13c, file="drug_reac_a13_full.txt", row.names=FALSE, quote=FALSE, sep=" ")
write.csv(dfdr13c, file="drug_reac_a13_full.csv", row.names=FALSE, quote=FALSE)


# see if there are any same ID's 
drug_reac_b13bc <- drug_reac_b13b %>% rename(primaryid = ISR_id)
drug_reac_b13bc$primaryid <- as.numeric(drug_reac_b13bc$primaryid)
...
write.table(drug_reac2,file="drug_reac_full.txt", row.names=FALSE, quote=FALSE, sep=" ")
write.csv(drug_reac2, file="drug_reac_full.csv", row.names = FALSE, quote=FALSE)
